...
`GroupQuestionResponse` SHALL include `children: List<GroupChildResponse>` instead of `followUpQuestions: List<TextQuestionResponse>`. Each `GroupChildResponse` SHALL contain `id` (UUID), `questionText`, `keywords`, and `marks`. `totalMarks` SHALL equal the sum of `children[].marks`.
...
- **THEN** each object in the `children` array contains `id`, `questionText`, `keywords`, and `marks`
