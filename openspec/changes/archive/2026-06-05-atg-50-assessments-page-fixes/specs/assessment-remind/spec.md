...
`AssessmentSummaryResponse` SHALL include an `invitationLink` field containing the full candidate-facing URL (`{frontendBaseUrl}/assessment/access/{token}`). The field MAY be null if the assessment has no token (should not occur in practice).
